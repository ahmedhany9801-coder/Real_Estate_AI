...
if page == 'Prediction':
    ...
    if predict_btn:
        ...
        if sqft_living >= sqft_lot:
            st.error("⚠️ في مشكلة: لازم تخلي مساحة البيت أصغر من مساحة الأرض.")
    ...
    with st.expander("Market Insights🏡"):
        ...
        st.markdown("##### Avg Price by Floors")
        avg_floors = df.groupby('num_floors')['price'].mean()
        fig_floors = px.bar(avg_floors)
